...
> - `ReviewRole`: `name` (string), `scope` (enum: `division`/`sub_division`), `raiseable_on_behalf_eligible` (boolean — only meaningful for `sub_division` scope), `system_role` (boolean). Seed: Dean and Associate Dean (`scope: division`, `system_role: true`), Supervisor (`scope: sub_division`, `system_role: true`). System roles can't be deleted by admin; everything else can.
